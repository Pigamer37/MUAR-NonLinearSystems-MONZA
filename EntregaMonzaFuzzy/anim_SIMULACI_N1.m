function anim_SIMULACI_N1(dif)
%ANIM_SIMULACI_N1  Animación de la moneda bajando por la pista con la planta suministrada.
%   anim_SIMULACI_N1(d)  ->  dificultad d (1..4). Por defecto 1.
%
%   Se lanza MONZA_fuzzy_delia_3.slx (planta TFM_SIMULACI_N1) con el fuzzy
%   fisController_simulink_dif{d}.fis, se recoje el giro del tablero y la pose
%   inercial de la moneda (giro_m, xs, ys) y se dibuja la animacion con
%   anim_giro_pistas: las pistas giran con el tablero y la moneda va en su
%   posicion real. La geometria sale de dificultad{d}.mat.

    if nargin < 1 || isempty(dif), dif = 1; end
    dif = min(max(round(dif), 1), 4);

    mdl      = "MONZA_fuzzy_delia_3";
    fisFile  = sprintf('fisController_simulink_dif%d.fis', dif);
    matFile  = sprintf('dificultad%d.mat', dif);     % geometria de la pista
    Ts       = 0.033;
    stopTime = 20;

    % El bloque Fuzzy lee la variable 'fisController' del workspace.
    ...
    % Señales que loguea el plano: giro del tablero y pose inercial
    giro_m = colData(getVar(out, 'giro_m'), 1);
    xs     = colData(getVar(out, 'xs'),     1);
    ys     = colData(getVar(out, 'ys'),     1);
    ...
    figure('Name', sprintf('MONZA - planta suministrada, dif %d', dif), 'Color', 'w');
    estilo = {'b.-', 'LineWidth', 1.5, 'MarkerSize', 4};
    for j = 1:n
        clf; hold on;

        % pistas giradas con el tablero. Uso marcadores '.' porque las parabolas
        % son muy planas y con linea fina sola el render se las traga.
        ...
        title(sprintf('SIMULADOR GRAFICO PLATAFORMA - MONZA (dif %d)', dif));
        drawnow;
...
% Imprime hasta que piso llego la moneda (Y de pos_ref, marco tablero).
function reportaPiso(out, Ts)
    yRef = colData(getVar(out, 'pos_ref'), 2);
    if isempty(yRef), return; end
    offsets = [0.1143, 0.0686, 0.03, -0.03, -0.0686, -0.1143, -0.16];
    tol  = 0.02;
    piso = min(max(nnz(offsets >= (min(yRef) - tol)), 1), numel(offsets));
    if piso >= numel(offsets)
        fprintf('Resultado: la moneda llega al piso %d en ~%.2f s.\n', piso, numel(yRef) * Ts);
    else
        fprintf('Resultado: la moneda llega al piso %d.\n', piso);
    end
end
